...
**Stage 2 — recognise.** Each polygon is perspective-warped to an upright crop
and passed to the recogniser. Warping matters: a plain bounding-box crop of text
at 30° carries as much background as text, and recognisers trained on upright
lines degrade badly on it.
...
The stages are kept separate on purpose. A single `readtext()` call hides which
one failed: a missing word is a detection failure, a garbled word is a
recognition failure, and the fixes are unrelated. Run `--stage detect` to draw
stage-1 boxes only and see immediately which one you are looking at.
...
| `--resize` | `1280` | resize long side before OCR (0 = off) |
...
**Nothing detected.** Check `--stage detect` first — if the boxes are there,
it's recognition, so lower `--min-conf`. If they're not, the text is too small:
raise `--resize` to 1920, or crop the frame to the region that matters.
